...
export const FormLayoutSlider = ({ layoutPath, path, dataPath, schema }) => {
  const shouldReload = (ov, nv) => {
    if (ov.timestamp[layoutPath] !== nv.timestamp[layoutPath]) return false;
    return true;
  }
  const { getSchemaItem, getError, getItemValue, updateError, timestamp } = useFormStore(state => state, shouldReload)
  ...
  return (
    <div className=' w-full'>
      ...
      <div className='text-xs flex flex-wrap gap-2 w-full mt-2 px-2'>
        <button title='Slide' onClick={e => makeActiveSlide(e, -1)} className={classNames(slideIndex < 0 && 'bg-yellow-200', ' h-5 w-5 flex items-center justify-center  border-cyan-400 border rounded p-0 hover:bg-cyan-300')}><Icon name='GrPlay' /></button>
        <button title='Previous' onClick={skipPrev} className={classNames(' h-5 w-5 flex items-center justify-center  border-cyan-400 border rounded p-0 hover:bg-cyan-300')}><Icon name='PiSkipBack' /></button>
        <button title='Next' onClick={skipNext} className={classNames(' h-5 w-5 flex items-center justify-center  border-cyan-400 border rounded p-0 hover:bg-cyan-300')}><Icon name='PiSkipForward' /></button>
        {layout?.items?.map((_, idx) => <button title={'Slide ' + 1} onClick={e => makeActiveSlide(e, idx)} className={classNames(slideIndex === idx && 'bg-yellow-200', ' h-5 w-5 flex items-center justify-center border-cyan-400 border rounded p-0 text-[11px] hover:bg-cyan-300')} >{idx + 1}</button>)}
      </div>
    </div>
  );
};
